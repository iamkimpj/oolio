...
import type { OolioConfig, RequestOptions, Routes, ApiClient } from "./types";
    ...
    for (const category in routes) {
      ...
      for (const fnName in routes[category]) {
        ...
        api[category][fnName] = (...args: any[]) => {
          const requestFn = setRequest(baseUrl, getAuthorizeToken, option, interceptors);

          let callOptions: RequestOptions = {};
          let positionalArgs = args;
          const lastArg = args[args.length - 1];
          if (lastArg !== null && typeof lastArg === "object" && "headers" in lastArg) {
            callOptions = lastArg;
          ...
          if (hasParams(route.path)) {
            ...
            return requestFn(route, pathParams, data, headers);
          } else {
            const [data = null] = positionalArgs;
            return requestFn(route, {}, data, headers);
          }
        };
      }
    }
